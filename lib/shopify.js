...
class ShopifyClient {
  ...
  async getUSPSFulfillments(daysBack) {
    ...
    // Flatten to one row per tracking entry, filtered to orders with Ember vendor (exact) excluding EMBERGC SKUs
    const fulfillments = [];
    for (const order of allOrders) {
      const serviceLevel = order.shippingLines?.edges?.[0]?.node?.title || null;
      for (const fulfillment of (order.fulfillments || [])) {
        const fulfillmentLineItems = (fulfillment.fulfillmentLineItems?.edges || []).map(e => e.node.lineItem);
        const hasQualifyingLine = fulfillmentLineItems.some(li =>
          li?.vendor === 'Ember' && !li?.sku?.includes('EMBERGC')
        );
