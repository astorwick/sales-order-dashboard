...
// Ember's only active Shopify location ("On Hand Inventory (SEKO Global)") — confirmed via a
// live `locations` query. Shared here so the fulfillment-location and inventory-availability
// checks stay in sync with a single source of truth.
const FULFILLMENT_LOCATION_ID = 'gid://shopify/Location/44073998';
...
class ShopifyClient {
  ...
  async getUSPSFulfillments(daysBack) {
    ...
    // Flatten to one row per tracking entry, filtered to Ember's fulfillment location
    const fulfillments = [];
    for (const order of allOrders) {
      const serviceLevel = order.shippingLines?.edges?.[0]?.node?.title || null;
      for (const fulfillment of (order.fulfillments || [])) {
        if (fulfillment.location?.id !== FULFILLMENT_LOCATION_ID) continue;

        for (const tracking of (fulfillment.trackingInfo || [])) {
          const carrier = tracking.company || null;
          const isAmazon = carrier && /amazon/i.test(carrier);
          const shipmentStatus = (!fulfillment.displayStatus || (isAmazon && fulfillment.displayStatus === 'FULFILLED'))
            ? 'NOT_AVAILABLE'
  ...
  async getFulfilledOrders(daysBack = 7, options = {}) {
    const sinceDate = new Date();
    sinceDate.setDate(sinceDate.getDate() - daysBack);
    const sinceDateStr = sinceDate.toISOString();
    const sourceNameFilter = options.sourceName ? ` AND source_name:${options.sourceName}` : '';

    let allOrders = [];
    let cursor = null;
    let hasNextPage = true;

    console.log(`Shopify: Fetching fulfilled/partial orders since ${sinceDateStr}${options.sourceName ? ` (source_name:${options.sourceName})` : ''}...`);

    while (hasNextPage) {
      const afterClause = cursor ? `, after: "${cursor}"` : '';
      const query = `
        {
          orders(first: 250, query: "created_at:>='${sinceDateStr}' AND NOT fulfillment_status:unshipped AND NOT status:cancelled${sourceNameFilter}"${afterClause}) {
            pageInfo {
              hasNextPage
              endCursor
            }
    ...
    const filtered = allOrders
      ...
      .map(order => ({
        name: order.name,
        createdAt: order.createdAt,
        serviceLevel: order.shippingLines?.edges?.[0]?.node?.title || null
      }));
  ...
  // Bulk enrichment source for the backfill: Shopify orders placed in an explicit
  // [sinceISO, untilISO) window. UNIS is what decides "this shipped" — this just supplies
  // createdAt/serviceLevel/sourceName for whatever po_no UNIS already told us about. Filtered
  // to non-unshipped/non-cancelled orders (same filter the live routes already use) since
  // anything UNIS confirms shipped will already be in that state on the Shopify side — this
  // cuts pull volume dramatically (unfiltered pulls ~5-10x more orders than are ever usable).
  async getFulfilledOrdersInRange(sinceISO, untilISO) {
    let allOrders = [];
    let cursor = null;
    let hasNextPage = true;

    console.log(`Shopify: Fetching orders from ${sinceISO} to ${untilISO}...`);

    while (hasNextPage) {
      const afterClause = cursor ? `, after: "${cursor}"` : '';
      const query = `
        {
          orders(first: 250, query: "created_at:>='${sinceISO}' AND created_at:<'${untilISO}' AND NOT fulfillment_status:unshipped AND NOT status:cancelled"${afterClause}) {
            pageInfo {
              hasNextPage
              endCursor
            }
      ...
      const data = await withRetry(async () => {
        ...
        if (json.errors) {
          ...
          if (throttled) {
            ...
            return { retry: true, delayMs: Math.ceil(1000 / restoreRate * 1000) };
          }
          throw new Error(`Shopify GraphQL error: ${json.errors[0].message}`);
        }

        return { retry: false, value: json };
      });
    ...
    console.log(`Shopify: ${allOrders.length} orders from ${sinceISO} to ${untilISO}`);
    ...
    return allOrders.map(order => ({
      ...
      freightPaid: order.totalShippingPriceSet?.shopMoney?.amount != null
        ? Number(order.totalShippingPriceSet.shopMoney.amount)
        : null
    }));
  }

  // Targeted enrichment for the ongoing sync: look up a small, specific batch of order
  // names directly rather than guessing a date window (the recurring sync only just
  // discovered these po_nos from a fresh UNIS shipment, so a name-based lookup sidesteps
  // any assumption about how far back the order might have been placed).
  async getOrdersByNames(names) {
    ...
    for (let i = 0; i < names.length; i += CHUNK_SIZE) {
      ...
      const nameQuery = chunk.map(n => `name:${n}`).join(' OR ');

      const query = `
        {
          orders(first: ${chunk.length}, query: "${nameQuery}") {
            edges {
              node {
                name
                createdAt
  ...
  // Batch inventory lookup for the Unshipped Orders tab's "OOS" possible-cause check.
  // Keyed by exact ProductVariant id (via Shopify's `nodes` root field), not SKU — some SKUs
  // in this store resolve to multiple distinct variants (confirmed live, e.g. legacy/duplicate
  // product listings), so a SKU-based search query would be ambiguous about which variant's
  // stock to report. `nodes(ids: [...])` fetches each variant unambiguously in one batch call.
  async getAvailableInventoryForVariantIds(variantIds) {
    ...
    for (let i = 0; i < uniqueIds.length; i += CHUNK_SIZE) {
      ...
      const query = `
        {
          nodes(ids: ${JSON.stringify(chunk)}) {
            ... on ProductVariant {
              id
              inventoryItem {
                inventoryLevel(locationId: "${FULFILLMENT_LOCATION_ID}") {
      ...
      for (const node of data.data.nodes) {
        if (!node) continue; // deleted/inaccessible variant
        const available = node.inventoryItem?.inventoryLevel?.quantities?.find(q => q.name === 'available')?.quantity;
        // No InventoryLevel row for this location (e.g. item not stocked/tracked there) —
        // leave it unset rather than assuming 0, so it's not misreported as OOS.
        if (available !== undefined) result.set(node.id, available);
      }
    }
