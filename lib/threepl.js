...
class ThreePLClient {
  ...
  async getOrderStatus(poNo) {
    try {
      ...
      return {
        ...
        // Timestamps
        createdAt: order['Created Date'] ? `${order['Created Date']} ${order['Create Time'] || ''}`.trim() : null,
        shippedDate: order['Shipped Time'] || order.shippedTime || null,
        pickedTime: order['PICKED TIME'] || order.pickedTime || null,

        // Shipping details
        carrier: order.Carrier || order.carrier,
        trackingNumber: order['Tracking No'] || order.trackingNo || null,
  ...
  async getOrderStatuses(poNumbers, daysBack = 7) {
    ...
    // Query all orders in parallel (much faster than sequential batches)
    const startTime = Date.now();
